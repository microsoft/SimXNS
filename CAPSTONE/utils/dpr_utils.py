...
CheckpointState = collections.namedtuple("CheckpointState",
                                         ...
                                         'offset', 'epoch', 'encoder_params'])
                                         
def _save_checkpoint(args, model, optimizer, scheduler, step: int, prefix='') -> str:
    offset = step
    epoch = 0
    model_to_save = get_model_obj(model)
    cp = os.path.join(args.output_dir, prefix+'checkpoint-' + str(offset))
...
def load_states_from_checkpoint_ict(model_file: str):
    from torch.serialization import default_restore_location
    logger.warning('Reading saved model from %s', model_file)
    state_dict = torch.load(model_file, map_location=lambda s, l: default_restore_location(s, 'cpu'))
    logger.warning('model_state_dict keys %s', state_dict.keys())
    new_stae_dict = {}
