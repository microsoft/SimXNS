...
from co_training_generate_new_train import RenewTools
...
def get_new_dataset(args, model, global_step, renew_tools):
    ...
    with torch.no_grad():
        ...
        if is_first_worker():
            ...
            ground_truth_path = os.path.join(data_dir, 'msmarco-doctrain-qrels.tsv')
            renew_tools.get_question_topk(train_q, train_q_embed, train_q_embed2id, ground_truth_path,
                                          ...
                                          mode='train', step_num=global_step, is_paced=True)
            data_dir = os.path.abspath(os.path.dirname(args.dev_qa_path))
            ground_truth_path = os.path.join(data_dir, 'msmarco-docdev-qrels.tsv')
            renew_tools.get_question_topk(dev_q, dev_q_embed, dev_q_embed2id, ground_truth_path,
                                          gpu_index_flat, passage_embedding2id,
                                          mode='dev', step_num=global_step, is_paced=True)
        # torch.distributed.barrier()
...
def main():
    ...
    passages_ctx_path = os.path.join(args.passage_path, 'msmarco-docs.tsv')
    renew_tools = RenewTools(passages_ctx_path=passages_ctx_path, tokenizer=tokenizer,
                             output_dir=args.ann_dir, temp_dir=temp_slice_dir, max_doc_character=args.max_doc_character)
    dist.barrier()
