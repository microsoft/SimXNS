...
from model.models import Reranker, RobertaDot, HFRobertaEncoder
import transformers
...
from utils.MARCO_until_Doc import (
    Doc_v2Dataset
)
import collections
...
def get_roberta_reranker_components(args,**kwargs):
    encoder = HFRobertaEncoder.init_encoder(
        args,
    )
...
def train(args, model, teacher_model, tokenizer, global_step=0):
    ...
    if global_step != 0:
        ...
        train_dataset = Doc_v2Dataset(train_data_path, tokenizer, num_hard_negatives=args.number_neg, a=args.a, b=args.b,
                                           trainer_id=args.local_rank, trainer_num=args.world_size,
                                           corpus_path=args.passage_path, rand_pool=100)

    else:
        train_dataset = Doc_v2Dataset(args.origin_data_dir, tokenizer, num_hard_negatives=args.number_neg, a=args.a, b=args.b,
                                           trainer_id=args.local_rank, trainer_num=args.world_size,
                                           corpus_path=args.passage_path, rand_pool=100)
    train_sample = RandomSampler(train_dataset)
    train_dataloader = DataLoader(train_dataset, sampler=train_sample,
                                  collate_fn=Doc_v2Dataset.get_collate_fn(args),
    ...
    while global_step < args.max_steps:
        ...
        inputs_student_query = {"input_ids": batch_student[0].long().to(args.device),
                          "attention_mask": batch_student[1].long().to(args.device)}
        inputs_student_doc = {"input_ids": batch_student[2].long().to(args.device),
                          "attention_mask": batch_student[3].long().to(args.device)}
        ...
        if train_flag == 0:  # 0: 训练retriever：用teacher 蒸馏student
            ...
            if hasattr(model, 'module'):
                local_ctx_vectors = model.module.body_emb(**inputs_student_doc)
                local_q_vector = model.module.query_emb(**inputs_student_query)
            else:
                local_ctx_vectors = model.body_emb(**inputs_student_doc)
                local_q_vector = model.query_emb(**inputs_student_query)
...
def load_model(args):
    ...
    tokenizer = RobertaTokenizer.from_pretrained(
        "roberta-base",
        do_lower_case=True)
    config = RobertaConfig.from_pretrained(args.model_type, gradient_checkpointing=args.gradient_checkpointing)
    model = RobertaDot.from_pretrained(args.model_type, config=config)
    ...
    teacher_model = get_roberta_reranker_components(args)
