...
def dev_batcher(device, is_training=False):
    ...
    return batcher_f
def GettrainDataProcessingFn_distill(args, query_cache, passage_cache, number_neg=20, shuffle=True):
    def fn(line, i):
        line_arr = line.split('\t')
        qid = int(line_arr[0])
        pos_pid = int(line_arr[1])
        ...
        if len(neg_pids) < number_neg:
            hard_neg_pids = [random.randint(0, 20000000) for i in range(number_neg)]
            if pos_pid in hard_neg_pids:
                hard_neg_pids.remove(pos_pid)
            neg_pids.extend(hard_neg_pids)
        ...
        neg_data = [GetProcessingFn(args, query=False)(passage_cache[neg_pid], neg_pid)[0] for neg_pid in
                    neg_pids[:number_neg // 2]]
        hard_neg_data = [GetProcessingFn(args, query=False)(passage_cache[hard_neg_pid], hard_neg_pid)[0] for
                         hard_neg_pid in neg_pids[number_neg//2:number_neg ]]
        return (query_data, pos_data, neg_data, hard_neg_data)
    return fn

def load_stream_dataset(args):
    ...
    if args.origin_data_dir is not None:
        train_data_path = os.path.join(args.origin_data_dir, "train-data")
        '''contrastive loss dataloader'''
        with open(train_data_path, 'r') as f:
            training_data = f.readlines()
            train_dataset_distill = OriginDataset(training_data,
                                                GettrainDataProcessingFn_distill(args, query_cache,
                                                                                passage_cache,
                                                                                number_neg=args.number_neg))
    if args.ann_data_path is not None:
        ann_train_data_path = os.path.join(args.ann_data_path, "ann_training_data_0")
        with open(ann_train_data_path, 'r') as f:
            ann_training_data = f.readlines()
        ann_train_dataset = OriginDataset(ann_training_data,
                                        GettrainDataProcessingFn_distill(args, query_cache,
                                                                                passage_cache,
                                                                                number_neg=args.number_neg))
        if train_dataset_distill is None:
            train_dataset_distill = ann_train_dataset
        else:
            train_dataset_distill = train_dataset_distill + ann_train_dataset

    if args.adv_data_path is not None:
        adv_data_path = os.path.join(args.adv_data_path, 'train-data-adv')
        with open(adv_data_path, 'r') as f:
            training_data_adv = f.readlines()
        adv_train_query = os.path.join(args.adv_data_path, 'train-query-adv')
        adv_query_cache = EmbeddingCache(os.path.join(adv_train_query))
        adv_query_cache.open()
        train_adv_dataset_distill = OriginDataset(training_data_adv,
                                                    GettrainDataProcessingFn_distill(args, adv_query_cache,
                                                                                    passage_cache,
                                                                                    number_neg=args.number_neg))
        if train_dataset_distill is None:
            train_dataset_distill = train_adv_dataset_distill
        else:
            train_dataset_distill = train_dataset_distill + train_adv_dataset_distill
    return train_dataset_distill

def load_tradition_dataset(args,tokenizer):
    dataset = None
    if args.origin_data_dir is not None:
        train_data_path = os.path.join(args.origin_data_dir,'nq-train.json')
        dataset = TraditionDataset(train_data_path,tokenizer,num_hard_negatives=args.number_neg)
    if args.ann_data_path is not None:
        train_data_path = os.path.join(args.ann_data_path,'nq-ann-train.json')
        ann_dataset = TraditionDataset(train_data_path,tokenizer,num_hard_negatives=args.number_neg)
        if dataset is None:
            dataset = ann_dataset
        else:
            dataset = dataset + ann_dataset
    if args.adv_data_path is not None:
        train_data_path = os.path.join(args.adv_data_path,'nq-adv-hn-train.json')
        ann_dataset = TraditionDataset(train_data_path,tokenizer,num_hard_negatives=args.number_neg)
        if dataset is None:
            dataset = ann_dataset
        else:
