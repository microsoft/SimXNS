    ...
    def __getitem__(self, index):
        ...
        if pos_score==0:
            ...
            for pair in neg_pairs_list:
                ...
                neg_score = math.exp(-abs(neg_score - pos_score) * self.tau)
...
class Rocketqa_v3Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        pos_pairs_list = sample.pos_id.split(',')
        neg_pairs_list = sample.neg_id.split(',')
        neg_pairs_list = [(int(pair.split()[0]), float(pair.split()[1]), float(pair.split()[2])) for pair in neg_pairs_list]
        if self.is_training:
            pos_id, pos_score = random.choice(pos_pairs_list).split()
        else:
            pos_id, pos_score = pos_pairs_list[0].split()
        ...
        if pos_score==0:
            ...
            neg_candidates = []
            neg_scores = []
            min_dd_score = min([pair[-1] for pair in neg_pairs_list])
            for pair in neg_pairs_list:
                neg_id, _, dd_score = pair
                dd_score = math.exp(abs(dd_score - min_dd_score) * self.tau)
                neg_candidates.append(neg_id)
                neg_scores.append(dd_score)
