...
class HFElectraEncoder(ElectraModel):
    ...
    def init_encoder(cls, args, dropout: float = 0.1, model_type=None):
        if model_type is None:
            model_type = args.model_type
        cfg = ElectraConfig.from_pretrained(model_type)
        if dropout != 0:
            cfg.attention_probs_dropout_prob = dropout
            cfg.hidden_dropout_prob = dropout
        if int(transformers.__version__.split('.')[0]) >= 3:
...
class BiElectraEncoder(BiBertEncoder):
    def __init__(self, args):
        super(BiElectraEncoder, self).__init__(args)
        self.question_model = HFElectraEncoder.init_encoder(args)
        if hasattr(args, 'share_weight') and args.share_weight:
            self.ctx_model = self.question_model
        else:
            self.ctx_model = HFElectraEncoder.init_encoder(args)
